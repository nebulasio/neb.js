 ...
 *
 * @param {Object} options
 * @param {Array|String} options.topic
 *
 * @return [eventData]{@link https://github.com/nebulasio/wiki/blob/master/rpc.md#subscribe}
 *
 * @example
 * var api = new Neb().api;
 * api.subscribe({topic: ["chain.linkBlock", "chain.pendingTransaction"]}).then(function(eventData) {
 * //code
 * });
 */
API.prototype.subscribe = function (options) {
    options = utils.argumentsToObject(['topic'], arguments);
    var params = { "topic": options.topic };
    return this._sendRequest("post", "/subscribe", params);
};
