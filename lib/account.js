...
Account.prototype = {
    ...
    toKey: function (password, opts) {
        ...
        if (kdf === 'pbkdf2') {
            kdfpropertys.c = opts.c || 262144;
            kdfpropertys.prf = 'hmac-sha256';
            derivedKey = cryptoUtils.crypto.pbkdf2Sync(new Buffer(password), salt, kdfpropertys.c, kdfpropertys.dklen, 'sha256');
        } else if (kdf === 'scrypt') {
            kdfpropertys.n = opts.n || 4096;
            kdfpropertys.r = opts.r || 8;
            kdfpropertys.p = opts.p || 1;
            derivedKey = cryptoUtils.scrypt(new Buffer(password), salt, kdfpropertys.n, kdfpropertys.r, kdfpropertys.p, kdfpropertys.dklen);
        } else {
    ...
    fromKey: function (input, password, nonStrict) {
        ...
        if (json.crypto.kdf === 'scrypt') {
            kdfpropertys = json.crypto.kdfpropertys;
            derivedKey = cryptoUtils.scrypt(new Buffer(password), new Buffer(kdfpropertys.salt, 'hex'), kdfpropertys.n, kdfpropertys.r, kdfpropertys.p, kdfpropertys.dklen);
        } else if (json.crypto.kdf === 'pbkdf2') {
            kdfpropertys = json.crypto.kdfpropertys;
            if (kdfpropertys.prf !== 'hmac-sha256') {
                throw new Error('Unsupported propertyeters to PBKDF2');
            }
            derivedKey = cryptoUtils.crypto.pbkdf2Sync(new Buffer(password), new Buffer(kdfpropertys.salt, 'hex'), kdfpropertys.c, kdfpropertys.dklen, 'sha256');
        } else {
        ...
        var decipher = cryptoUtils.crypto.createDecipheriv(json.crypto.cipher, derivedKey.slice(0, 16), new Buffer(json.crypto.cipherpropertys.iv, 'hex'));
