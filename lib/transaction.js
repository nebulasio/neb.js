 ...
 * @param {Number} options.chainID - Transaction chain id.
 * @param {Account} options.from - Hex string of the sender account addresss..
 * @param {Account} options.to - Hex string of the receiver account addresss..
 * @param {Number} options.value - Value of transaction.
 * @param {Number} options.nonce - Transaction nonce.
 * @param {Number} options.gasPrice - Gas price. The unit is 10^-18 NAS.
 * @param {Number} options.gasLimit - Transaction gas limit.
...
var Transaction = function (options) {
    options = utils.argumentsToObject(['chainID', 'from', 'to', 'value', 'nonce', 'gasPrice', 'gasLimit', 'payload'], arguments);
    ...
    this.data = parsePayload(options.payload);
